$(document).ready(function(){
	...
	$('#note-submit').live('click',function(e){
		...
		/* The submit button: */
	$('#select-submit').live('click',function(e){
		...
		var data = {
			'zindex'	: ++zIndex,
			'body'		: $('.sel_body').find("option:selected").text(),
			'author'		: $('.pr-author').val(),
			'color'		: $.trim($('#fancy_ajax .note').attr('class').replace('note',''))
		};
		...
		$.post('ajax/post.php',data,function(msg){
			...
			if(parseInt(msg))
			{
				...
				tmp.find('span.data').text(msg).end().css({'z-index':zIndex,top:10,left:20});
				tmp.appendTo($('#main'));
